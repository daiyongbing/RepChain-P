      ...
      """
{
  ...
  "amount" : 50
    ...
    //构建测试用的交易
    var num = 0
    val out = new FileWriter("d:/num.txt", true)
    val out1 = new FileWriter("d:/put_proof.txt", true)
    for (i <- 1 to 150) {
      num += 1
      //加载put_proof合约调用脚本
      var l4 =
        """
      {
        "c3cb88031887e258c2559a463fe60952bdb585d0a708ae1eeb51a4e4722836ad$num":""
      }
      """
      l4 = l4.replace("$num", num + "")
      println(l4)
      val t5 = transactionCreator(sysName, rep.protos.peer.Transaction.Type.CHAINCODE_DEPLOY,
        "", "", List(), l1 + num, None, rep.protos.peer.ChaincodeSpec.CodeType.CODE_SCALA)
      println(t5.toByteArray)
      out.write('"' + encodeHex(t5.toByteArray) + '"' + "\r\n")
      // val cname = t1.payload.get.chaincodeID.get.name
      val cname = "555ae6a3e223eba89543b3d3e9be6837de3e14dfdd588d8796f5b462efe0599f"
      val t6 = transactionCreator(sysName, rep.protos.peer.Transaction.Type.CHAINCODE_INVOKE,
        "", "put_proof", Seq(l4), "", Option(cname))
      out1.write('"' + encodeHex(t6.toByteArray) + '"' + "\r\n")
